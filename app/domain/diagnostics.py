...
def generate_recommendations(result: SunExposureResult) -> list[str]:
    ...
    if not result.has_sun:
        ...
        tips.append("Plantas que toleram sombra são mais adequadas para este espaço.")
        return tips
    ...
    if orientation == FacadeOrientation.LESTE:
        ...
        tips.append("Considere proteção solar no verão para evitar superaquecimento.")
    ...
    if total_hours >= 6.0:
        ...
        tips.append(f"Boa exposição solar: {total_hours:.1f}h de sol direto por dia.")
    else:
        tips.append(

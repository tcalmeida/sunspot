...
- [ ] 8. Implementar `app/services/solar.py` com dataclass `SolarData` e função `calculate_solar_position(latitude, longitude, date, interval_minutes) -> SolarData` usando `pvlib.solarposition.get_solarposition` e `pandas.date_range` com timezone-aware; tratar erros lançando `SolarCalculationError`
- [ ] 9. Implementar `app/domain/diagnostics.py` com enum `FacadeOrientation` (8 octantes), dataclass `SunExposureResult`, e as funções `classify_orientation`, `generate_recommendations` e `calculate_sun_exposure`; aplicar os três filtros de interseção solar (noturno, horizonte e angular); garantir que nenhum import de `streamlit`, `geopy` ou `pvlib` existe neste módulo
- [ ] 10. Implementar `app/ui/main_page.py` com as funções `render_inputs`, `render_results`, `render_error` e `render_main_page`; orquestrar o fluxo completo com `st.spinner`, validação de inputs e tratamento de erros amigável
- [ ] 11. Finalizar `app.py` com `st.set_page_config` e chamada a `render_main_page()`; executar `ruff check .`, `black --check .` e `mypy .` sem erros
